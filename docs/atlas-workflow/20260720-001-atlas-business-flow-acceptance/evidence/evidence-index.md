...
- 当前 Core evidence ref 只有 bundle 内路径和 SHA-256 等运行完整性字段，没有稳定 artifact locator、size、media type、sensitivity、retention class 或 policy reference。
- live/visualization evidence 依赖 `/home/gewu/.codex/...` 本机绝对路径，尚未证明跨目录、跨机器或清理后可恢复。
...
- Git 只保留 manifest/digest、规则、结论和小型脱敏 fixtures/golden；大体积或敏感 raw evidence 进入受控 durable storage，调试材料留 ephemeral。
- accepted durable baseline 要求完整 manifest，并从稳定 locator 恢复到新临时目录后重新通过 digest、v2 review 和 strict closure。
- Core 不选择 storage provider、不上传或删除；实际 storage mutation 等待用户选择 target 与新增权限。
