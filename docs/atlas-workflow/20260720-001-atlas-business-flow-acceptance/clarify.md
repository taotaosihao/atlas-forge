...
- 大体积或敏感原件不进入 Git；Git 只保留规则、实现、最小脱敏样例和结论。外部 durable storage、export/retention、provider locator/resolver、fresh-root/hermetic、DR 与长期防篡改属于后续 experimental/pending，不阻塞当前 human-first v2 材料。
...
- 若修改当前 accepted artifact，会破坏历史；因此迁移生成新的 review bundle，并保持历史 artifact 只读不变。
- raw evidence 不进入 Git，当前材料仅引用当前 bundle 已登记 evidence；外部长期保存能力未登记且不作为本合同阻塞条件。
