...
v1 只证明一条可信闭环：计划员从真实登录页进入，通过 UI 创建并发布工单；无效 Beezer 签名被拒绝且任务状态不变；有效 signed callback 经真实 ingress 推动同一 `DeviceTask` 到 `running`；UI、API、DB、callback、审计和 trace 一致。任何缺证据、首试失败、局部通过或 non-claim 都不能成为最终业务通过。
...
- 中文审核材料是确定性派生物并绑定 digest；它不重算 verdict，也不等于生产发布批准。
