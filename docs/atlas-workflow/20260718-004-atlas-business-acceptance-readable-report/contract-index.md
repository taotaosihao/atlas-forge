...
- repair_user_request: 修订 Team Review findings 并再次 review
...
- blocking_team_review: external workflow task `20260718-006-atlas-team-review` decision `BLOCK`
- completed_repair_review: external workflow task `20260718-007-atlas` final three-seat decision `PASS`

authority_rules:
- `implementation-contract.final.md` 是后续实现范围、验收、验证和停止条件的唯一权威。
- 合同和报告中同名的“验收前先看四件事”都是强制合同，不是可选文案。
...
next_action:
- 等待用户明确授权实施当前权威合同；修订与三席复审本身不授权编写 renderer、刷新安装态或发布。
