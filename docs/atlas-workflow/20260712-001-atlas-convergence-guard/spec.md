...
1. Execute 缺明确实施消息引用时无副作用失败；discuss 与非 execute promotion 兼容。
...
5. Reviewer 发现自由、自动 repair 有限；full-roadmap 连续执行；commit 适中自动化；checkpoint 单一覆盖写。
6. Checkout、Multica、release 和最小验证边界保持，已有用户改动不被覆盖或夹带提交。
