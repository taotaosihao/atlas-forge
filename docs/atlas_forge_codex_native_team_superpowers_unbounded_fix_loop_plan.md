# Atlas Forge Codex Native Team × Superpowers SDD 完整修订方案（Fix Loop 无上限版）

> 目标：把 `atlas-forge` 的 Codex native team 从“能调度 subagent 的团队技能”升级为可恢复、可审计、可验证的 **Codex-native SDD controller**。本版吸收了事实核查后的修正：subagent 不直接写 workflow artifacts，fix loop 无固定次数上限，implementer 强制 commit，ledger 进入 MVP，schema 采用无依赖可实现子集，硬编码路径治理按全仓规模处理，全文路径统一为 `$WORKFLOW_ROOT/artifacts/...`。
...
### 0.4 fix loop 改为无固定次数上限

本版按要求采用 **unbounded fix loop**：只要 reviewer 仍给出 Critical / Important finding 或 `spec_compliance = fail`，controller 就继续派发 fixer、要求 commit、重新生成 review package、重新 review。
...
fix loop 不再因为迭代次数达到某个数字而停止，也不再产生旧的 exhausted stop 事件。合法终止条件只有：

- review clean：`spec_compliance = pass` 且 `task_quality = pass`；
- fixer / implementer 返回真实 `BLOCKED`，且 controller 无法通过补上下文、拆 slice、换模型或修正 brief 解决；
- `NEEDS_CONTEXT` questions loop 超过 `max_question_rounds`；
- 连续修复没有产生实质新 diff / 新 commit，或同一个 blocking finding 在有证据的情况下反复原样出现，controller 记录 `fix_progress_stalled` 并升级 human；
- 人类或 controller 明确中止；
- 环境失败，例如测试环境不可用、git 状态损坏、native subagent runtime 不可用。

Atlas 仍保留 ledger、iteration 编号、scorecard 和 explicit terminal state 来防止“静默空转”，但不设置 synthetic iteration cap。
...
```text
Codex native subagents + Superpowers SDD protocol + Atlas workflow artifacts + unbounded fix loop + resumable ledger + no-dependency validators
```
...
4. Critical / Important findings 必须修复并重审；fix loop 无固定次数上限，直到 review clean 或进入明确 terminal state。
...
| 无限 fix loop | 保留无固定次数上限；用 ledger、terminal state、scorecard 观测和人工中止替代迭代上限 | 按本次要求忠实采用 Superpowers 风格持续修复 |
...
- Native Bounded Loop 的 verification gate、terminal status 与记录机制；但 SDD 的 per-slice fix loop 不使用 `max_iterations` 作为停止条件。
...
- `max_question_rounds` 是正整数，默认 2。
- `fix_loop_policy` enum：`unbounded_until_clean_or_terminal`。不得使用固定次数上限字段作为 SDD gate。
...
14. Run fix loop with no fixed iteration cap; each fix must commit and re-review. Stop only on review clean or explicit terminal state.
15. Resolve `cannot_verify_from_diff` items before completion.
16. Append `slice_complete` only after spec and quality pass.
17. Continue to next slice without asking the user unless blocked, ambiguous, question-loop-exhausted, explicitly stopped, or all slices are complete.
...
Every SDD slice uses an unbounded fix loop for review failures. There is no fixed-iteration gate. The controller records every iteration in the ledger, requires a commit for each file-changing fix, regenerates the review package, and re-reviews until the slice is clean or reaches an explicit terminal state such as `BLOCKED`, `question_loop_exhausted`, `fix_progress_stalled`, `human_aborted`, `controller_aborted`, or `environment_failed`.
```
...
含义：reviewer 只要仍返回 Critical / Important finding 或 `spec_compliance = fail`，controller 就必须继续派发 fixer 并 re-review；不得因为 fix iteration 达到固定次数而停止。
...
无 exhausted-by-iteration 事件。若必须停止，必须记录真实 terminal reason，例如：

```jsonl
{"event":"fix_progress_stalled","task_id":"T-123","slice_id":"slice-001","iteration":7,"reason":"same Important finding persisted with no material diff after repeated fix attempts"}
{"event":"slice_blocked","task_id":"T-123","slice_id":"slice-001","reason":"unresolved_blocker","iteration":7}
{"event":"escalated_human","task_id":"T-123","slice_id":"slice-001","reason":"fixer cannot resolve remaining blocking findings after repeated attempts","iteration":7}
```

允许的 terminal reason：`review_clean`、`unresolved_blocker`、`question_loop_exhausted`、`fix_progress_stalled`、`human_aborted`、`controller_aborted`、`environment_failed`。其中 `fix_progress_stalled` 不是次数上限，而是基于证据的无进展判定：例如连续多轮没有新 commit、reviewer finding 完全相同且 fixer 无法解释差异，或测试/产品 oracle 本身冲突。
...
| `fix-loop-unbounded.fixture` | 至少 3 次 fix 仍继续 re-review，直到 review clean；不得出现 exhausted-by-iteration 事件 |
...
- Specify questions loop、fresh context、continuous execution、unbounded fix loop。
...
| fix loop 无上限 | 成本上升或重复修复 | ledger 记录每轮、scorecard 统计、reviewer 必须给 file:line evidence、controller 可在真实 `BLOCKED` / human abort / environment failure 时停止；不使用 synthetic iteration cap |
...
9. For each slice:
   ...
   l. fix Critical/Important/spec failures with unbounded fix loop
