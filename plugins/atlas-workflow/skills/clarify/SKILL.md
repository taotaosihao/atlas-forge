...
- Release certification v1 supports only a pure Web UI with immutable Profile `web-ui-v1`. API, CLI, worker, mixed, and unknown product surfaces must fail authoring/admission until a dedicated Profile exists; report their requested release conclusion as `cannot_verify` without inventing a completion `release_decision` or relabeling them as non-product.
