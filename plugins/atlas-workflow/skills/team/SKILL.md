...
- For new or renamed durable product files, directories, symbols, APIs, schemas, and capability identifiers, review stable domain/capability language separately from delivery-only task, ticket, Gate, phase, slice, agent, or acceptance labels. A neighboring legacy name does not authorize copying stage language. Keep explicit compatibility-bound migrations, persisted identifiers, receipts, evidence namespaces, and purpose-built verifier names out of ordinary product renames.
