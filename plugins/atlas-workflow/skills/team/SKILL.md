...
11. Check status or stop the active round with:
   - `~/.codex/workflow/bin/codex-workflow team-status <task-id>`
   - `~/.codex/workflow/bin/codex-workflow team-stop <task-id>`
12. Before promoting to execution, worktree, finish, or Multica handoff, run:
...
14. In the final reply, include the task id, `decision.md` path, staffing path if produced, readiness result if run, promotion state, and any open decision.
