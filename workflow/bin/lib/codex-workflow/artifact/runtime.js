...
function oneLine(value, label, { allowEmpty = true } = {}) {
  if (/[\n\r\t]/.test(value)) {
    throw new ArtifactError(`invalid ${label}: must be a single line`);
  }
  const trimmed = value.trim();
  if (!allowEmpty && !trimmed) {
    throw new ArtifactError(`invalid ${label}: must be non-empty`);
  }
  return trimmed;
}
...
function prepareArtifactTask(paths, taskId, clock) {
...
function updateArtifactTask(paths, taskId, headerUpdates, stateUpdates, clock) {
  const file = requireTaskFile(paths.tasksDir, taskId);
