...
function parse(argv, command) {
  ...
  const allowed = command === "run" ? ["projectConfig", "contract", "artifactRoot", "runId", "attempts", "format"] : command === "check-run" ? ["runRoot", "format"] : ["bafRoot", "card", "contract", "flowContract", "format", "checkOwnerDecision"];
  for (const key of Object.keys(result)) if (!allowed.includes(key)) throw new ContractError(`未知选项: --${key}`);
  result.format ||= "human"; if (!new Set(["json", "human", "markdown"]).has(result.format)) throw new ContractError("--format 必须是 json、human 或 markdown"); if (result.format === "markdown" && command !== "review") throw new ContractError("--format markdown 仅支持 review");
...
function main(argv) {
  ...
  try { const options = parse(argv.slice(1), command); const value = command === "run" ? run(options) : command === "check-run" ? checkRun(options) : review(options); if (options.format === "json") process.stdout.write(`${JSON.stringify(value)}\n`); else if (options.format === "markdown") process.stdout.write(value.markdown); else process.stdout.write(human(command, value)); const ok = command === "run" ? value.result.technical_status === "passed" : value.ok; process.exitCode = ok ? 0 : 2; }
