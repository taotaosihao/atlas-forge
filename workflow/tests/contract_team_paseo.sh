...
expect_fail "paseo record missing selection authority" "$BIN" team-record-start "$paseo_id" "paseo objective" --backend paseo --mode discuss --agents 2 --roles planner,reviewer
expect_fail "paseo record invalid providers" "$BIN" team-record-start "$paseo_id" "paseo objective" --backend paseo --mode discuss --agents 2 --roles planner,reviewer --providers $'codex=gpt-5.6\nclaude=sonnet-4' --selection-authority-kind user-message --selection-authority-ref user-message:paseo-contract
$BIN team-record-start "$paseo_id" "paseo objective" --backend paseo --mode discuss --agents 5 --roles "planner,implementer,reviewer,verifier,integrator" --providers "codex=gpt-5.6 via openai; claude=sonnet-4 via anthropic; deepseek=deepseek-coder via volcengine; glm=glm-4.5 via zhipu; kimi=kimi-k3 via kimi" --selection-authority-kind user-message --selection-authority-ref user-message:paseo-contract >/dev/null
...
grep -q "team_configured_backend: paseo" "$TMP_ROOT/paseo-status-running.out"
grep -q "team_resolved_requested_backend: paseo" "$TMP_ROOT/paseo-status-running.out"
...
grep -q "team_backend: native" "$TMP_ROOT/paseo-status-complete.out"
grep -q "team_status: complete" "$TMP_ROOT/paseo-status-complete.out"
grep -q "team_staffing: " "$TMP_ROOT/paseo-status-complete.out"
grep -q "team_effective_backend: none" "$TMP_ROOT/paseo-status-complete.out"
test -f "$paseo_team_dir/backend-v2.json"
paseo_loop_id="$($BIN init-task "contract paseo team loop" "paseo team loop")"
$BIN start "$paseo_loop_id"
write_ready_artifacts "$paseo_loop_id"
$BIN team-record-start "$paseo_loop_id" "paseo loop objective" --backend paseo --mode discuss \
  --selection-authority-kind user-message \
  --selection-authority-ref user-message:paseo-loop-contract >/dev/null
paseo_loop_team_dir="$CODEX_WORKFLOW_ROOT/artifacts/$paseo_loop_id/team"
paseo_loop="$paseo_loop_team_dir/loop-paseo.md"
wrong_paseo_loop="$paseo_loop_team_dir/loop-native.md"
printf '%s\n' '# Wrong Paseo Team Loop' '' '- backend: native' '' '## Evidence' 'A native marker cannot close a Paseo loop.' > "$wrong_paseo_loop"
expect_fail "paseo loop native marker" "$BIN" team-loop-record "$paseo_loop_id" --backend paseo --status loop-done --loop "$wrong_paseo_loop" --iterations 1
...
$BIN team-loop-record "$paseo_loop_id" --backend paseo --status loop-done --loop "$paseo_loop" --iterations 2 --max-time 12m >/dev/null
$BIN team-status "$paseo_loop_id" > "$TMP_ROOT/paseo-loop-status.out"
...
grep -q "team_effective_backend: none" "$TMP_ROOT/paseo-loop-status.out"

paseo_dynamic_id="$($BIN init-task "contract paseo dynamic staffing" "paseo dynamic staffing")"
$BIN start "$paseo_dynamic_id"
write_ready_artifacts "$paseo_dynamic_id"
$BIN team-record-start "$paseo_dynamic_id" "paseo dynamic objective" --backend paseo --mode discuss --selection-authority-kind user-message --selection-authority-ref user-message:paseo-dynamic >/dev/null
$BIN team-status "$paseo_dynamic_id" > "$TMP_ROOT/paseo-dynamic-status.out"
grep -q "team_agents: " "$TMP_ROOT/paseo-dynamic-status.out"
grep -q "team_roles: " "$TMP_ROOT/paseo-dynamic-status.out"
grep -q "team_providers: " "$TMP_ROOT/paseo-dynamic-status.out"
