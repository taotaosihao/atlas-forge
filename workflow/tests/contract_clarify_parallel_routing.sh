...
assert_has "$CLARIFY" 'Ask a question when its answer could change the recommendation' 'valuable questions are distinguished from blocking waits'
assert_has "$CLARIFY" 'Mark it blocking only when proceeding would overreach' 'blocking waits require authority or cost evidence'
assert_has "$CLARIFY" 'chosen direction is a current decision, not proof' 'selected direction does not prove its premises'
...
assert_has "$CLARIFY" 'engineering adjustments the implementer may make' 'handoff preserves allowed engineering adjustment'
assert_has "$CLARIFY" 'applicable\s+acceptance and readback obligations' 'handoff preserves applicable acceptance'
assert_has "$CLARIFY" 'important unresolved items' 'handoff preserves important unresolved items'
