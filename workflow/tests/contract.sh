...
rg -U -q "Decide whether Team is[[:space:]]+needed from the user's current request" "$ATLAS_FORGE_ROOT/plugins/atlas-workflow/README.md"
rg -U -q "Multiple[[:space:]]+files, behavior changes, and task complexity do not by themselves require Team" "$ATLAS_FORGE_ROOT/plugins/atlas-workflow/README.md"
...
rg -q "explicitly asks for native collaboration" "$source_skills_root/team/SKILL.md"
...
rg -q "paseo provider models <provider> --json" "$source_skills_root/team/SKILL.md"
rg -q "Never read or apply Paseo orchestration preferences" "$source_skills_root/team/SKILL.md"
rg -q "latest available stable model" "$source_skills_root/team/SKILL.md"
rg -q "explicit user provider/model request wins" "$source_skills_root/team/SKILL.md"
! rg -q "orchestration-preferences\.json|matching role preference" "$source_skills_root/team/SKILL.md"
rg -q "Default Provider Assignment" "$source_skills_root/team/SKILL.md"
rg -q 'implementation and focused repair: `deepseek`' "$source_skills_root/team/SKILL.md"
rg -q 'latest stable `glm` model' "$source_skills_root/team/SKILL.md"
rg -q 'verification.*,.*`kimi` through Kimi Code CLI' "$source_skills_root/team/SKILL.md"
rg -q "Do not default every lane to Codex" "$source_skills_root/team/SKILL.md"
rg -q "reviewer and implementer providers distinct" "$source_skills_root/team/SKILL.md"
rg -q "paseo run --detach --json" "$source_skills_root/team/SKILL.md"
rg -q -- '--mode <full-access-mode-id>' "$source_skills_root/team/SKILL.md"
rg -q 'full-access.*Codex.*bypass.*Claude/DeepSeek.*yolo.*Kimi Code CLI' "$source_skills_root/team/SKILL.md"
rg -q "fail closed; do not silently start in a less permissive mode" "$source_skills_root/team/SKILL.md"
rg -q "Full-access runtime permission does not grant implementation authority" "$source_skills_root/team/SKILL.md"
rg -q "paseo wait <agent-id> --json" "$source_skills_root/team/SKILL.md"
rg -q -- "--providers" "$source_skills_root/team/SKILL.md"
rg -q "GLM" "$source_skills_root/team/SKILL.md"
rg -q "kimi provider explicitly" "$source_skills_root/team/SKILL.md"
