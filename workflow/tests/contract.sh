...
rg -q 'Default path: Luna implementer, then Terra routine reviewer and verifier' "$source_skills_root/team/SKILL.md"
rg -q 'after one non-mechanical review or verification failure whose cause is unclear' "$source_skills_root/team/SKILL.md"
rg -q 'only when it supports phase or final acceptance' "$source_skills_root/team/SKILL.md"
rg -q 'route the resulting diff and evidence to `atlas-sdd-phase-reviewer`' "$ATLAS_FORGE_ROOT/.codex/agents/atlas-sdd-browser-verifier.toml"
