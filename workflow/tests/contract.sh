...
test -x "$BIN"
test -x "$LEGACY_BIN"
bash -n "$ATLAS_FORGE_ROOT/workflow/tests/contract_native_agent_inbox.sh"
bash "$ATLAS_FORGE_ROOT/workflow/tests/contract_native_agent_inbox.sh"
pass "native agent inbox contract"
...
grep -q "### Admitted Lanes" "$scaffold_dir/team/staffing.md"
grep -q "Record only lanes that passed controller admission" "$scaffold_dir/team/staffing.md"
grep -q "Dispatch width used" "$scaffold_dir/team/staffing.md"
grep -q "## Ownership" "$scaffold_dir/team/staffing.md"
grep -q "Canonical scope/artifact writer: main Codex" "$scaffold_dir/team/staffing.md"
grep -q "## Record-Only Compatibility" "$scaffold_dir/team/staffing.md"
grep -q "effective_backend=none" "$scaffold_dir/team/staffing.md"
grep -q "## Verification" "$scaffold_dir/team/staffing.md"
grep -q "artifact_category: phase_projection_sentinel" "$scaffold_dir/evidence/phase-1/phase-review-report.md"
grep -q "Canonical 状态：未投影" "$scaffold_dir/evidence/phase-1/phase-review-report.md"
grep -q "codex-workflow project-phase-report" "$scaffold_dir/evidence/phase-1/phase-review-report.md"
...
bash -n "$ATLAS_FORGE_ROOT/workflow/tests/contract_team_paseo.sh"
bash -n "$ATLAS_FORGE_ROOT/workflow/tests/contract_team_fallback.sh"
...
bash -n "$ATLAS_FORGE_ROOT/workflow/tests/contract_release_prompt.sh"
bash -n "$ATLAS_FORGE_ROOT/workflow/tests/contract_release_agents.sh"
source "$ATLAS_FORGE_ROOT/workflow/tests/contract_team_native.sh"
source "$ATLAS_FORGE_ROOT/workflow/tests/contract_team_paseo.sh"
source "$ATLAS_FORGE_ROOT/workflow/tests/contract_team_fallback.sh"
...
bash "$ATLAS_FORGE_ROOT/workflow/tests/contract_web_acceptance.sh"
bash "$ATLAS_FORGE_ROOT/workflow/tests/contract_release_prompt.sh"
bash "$ATLAS_FORGE_ROOT/workflow/tests/contract_release_agents.sh"
rg -q 'formal release-ready/certified' "$ATLAS_FORGE_ROOT/plugins/atlas-workflow/.codex-plugin/plugin.json"
rg -q 'MVP/Beta/internal/small-beta without formal certification = product_increment' "$ATLAS_FORGE_ROOT/plugins/atlas-workflow/.codex-plugin/plugin.json"
rg -q 'Staffing, model, path lease, and release decisions stay independent' "$ATLAS_FORGE_ROOT/plugins/atlas-workflow/.codex-plugin/plugin.json"
rg -q 'do not certify, install, deploy, publish, or release a product' "$ATLAS_FORGE_ROOT/plugins/atlas-workflow/.codex-plugin/plugin.json"
rg -q 'Product increment versus product release' "$ATLAS_FORGE_ROOT/workflow/README.md"
rg -q '证据采集：降级' "$ATLAS_FORGE_ROOT/workflow/README.md"
rg -q 'does not create `release_decision`' "$ATLAS_FORGE_ROOT/workflow/README.md"
...
rg -U -q "Decide whether[[:space:]]+Team is[[:space:]]+needed from the user's current request" "$ATLAS_FORGE_ROOT/plugins/atlas-workflow/README.md"
rg -U -q "Multiple[[:space:]]+files, behavior changes,[[:space:]]+and task complexity do not by[[:space:]]+themselves require Team" "$ATLAS_FORGE_ROOT/plugins/atlas-workflow/README.md"
rg -U -q "Short[[:space:]]+requests,[[:space:]]+file[[:space:]]+counts[[:space:]]+and[[:space:]]+repetitive[[:space:]]+bulk[[:space:]]+changes[[:space:]]+do[[:space:]]+not[[:space:]]+by[[:space:]]+themselves[[:space:]]+require[[:space:]]+deliberation[[:space:]]+or[[:space:]]+new[[:space:]]+artifacts" "$ATLAS_FORGE_ROOT/plugins/atlas-workflow/README.md"
...
rg -q "Decide whether Team is needed from the user's current request" "$source_skills_root/team/SKILL.md"
rg -q "Multiple files, behavior changes, task complexity.*do not require Team" "$source_skills_root/team/SKILL.md"
rg -q "no default role set or required agent count" "$source_skills_root/team/SKILL.md"
rg -q "default to Codex native collaboration.*DeepSeek V4 Pro" "$source_skills_root/team/SKILL.md"
rg -q "Select Paseo only from an explicit user or operator choice" "$source_skills_root/team/SKILL.md"
rg -q 'dispatch, lane, Team, then `backend=native` and `fallback_policy=codex`' "$source_skills_root/team/SKILL.md"
rg -q "bounded parallel dispatch over the admitted ready frontier rather than main-first serial exploration" "$source_skills_root/team/SKILL.md"
rg -q "## Bounded-Parallel Controller Policy" "$source_skills_root/team/SKILL.md"
rg -q 'child_count = min\(ready independent lanes, host available child slots, 4\)' "$source_skills_root/team/SKILL.md"
! rg -q "Start with the main Codex, but prefer parallel native agents" "$source_skills_root/team/SKILL.md"
rg -q "disjoint path ownership" "$source_skills_root/team/SKILL.md"
...
# Prompt-policy lint only: executable Team behavior is covered by the native/paseo/SDD contracts above.
...
rg -q "moderate logical commits" "$source_skills_root/team/SKILL.md"
...
rg -q "paseo provider ls --json" "$source_skills_root/team/SKILL.md"
rg -q "Never read or apply Paseo orchestration preferences" "$source_skills_root/team/SKILL.md"
! rg -q "providers.atlas_deepseek_impl.*providers.atlas_deepseek_research" "$source_skills_root/team/SKILL.md"
rg -q "An explicit provider/model request wins" "$source_skills_root/team/SKILL.md"
rg -q "trusted capability identity is explicitly non-Claude" "$source_skills_root/team/SKILL.md"
rg -q "Unknown gateway aliases are never eligible for automatic recommendation" "$source_skills_root/team/SKILL.md"
rg -q "An exact attested unknown selection may proceed while remaining visibly unverified" "$source_skills_root/team/SKILL.md"
rg -q "Claude-family models are never eligible for automatic routing" "$source_skills_root/team/SKILL.md"
rg -q "CLAUDE_MODEL_SELECTION_REQUIRED" "$source_skills_root/team/SKILL.md"
rg -q "MODEL_FAMILY_UNVERIFIED" "$source_skills_root/team/SKILL.md"
rg -q 'do not guess `full-access`, `bypass`, `bypassPermissions`, `yolo`' "$source_skills_root/team/SKILL.md"
rg -q 'paseo run --provider claude --model "<exact-model-id>" --mode bypassPermissions' "$source_skills_root/team/SKILL.md"
rg -q 'shorten it to the display label `bypass`' "$source_skills_root/team/SKILL.md"
rg -q "Claude Code's lower-level.*--permission-mode.*in a Paseo command" "$source_skills_root/team/SKILL.md"
rg -q "launch-state-unknown" "$source_skills_root/team/SKILL.md"
rg -q "Atomically record the fallback event and reserve the native attempt" "$source_skills_root/team/SKILL.md"
rg -q 'backend: native|paseo|mixed|none' "$source_skills_root/team/SKILL.md"
...
rg -q 'agent_type.*model.*reasoning_effort.*fork_turns' "$source_skills_root/team/SKILL.md"
rg -q 'atlas-sdd-implementer.*gpt-5\.6-luna.*max.*none' "$source_skills_root/team/SKILL.md"
rg -q 'atlas-sdd-implementer-deepseek.*deepseek-v4-pro:deepseek.*max.*none' "$source_skills_root/team/SKILL.md"
rg -q 'atlas-sdd-reviewer.*gpt-5\.6-terra.*max.*none' "$source_skills_root/team/SKILL.md"
rg -q 'atlas-sdd-planner.*gpt-5\.6-sol.*high.*none' "$source_skills_root/team/SKILL.md"
rg -q 'atlas-sdd-phase-reviewer.*gpt-5\.6-sol.*medium.*none' "$source_skills_root/team/SKILL.md"
rg -q 'atlas-sdd-explorer-deepseek.*deepseek-v4-pro:deepseek.*max.*none' "$source_skills_root/team/SKILL.md"
rg -q 'Cross v1' "$source_skills_root/team/SKILL.md"
rg -q 'generation-local' "$source_skills_root/team/SKILL.md"
rg -q 'controller-enforced' "$source_skills_root/team/SKILL.md"
rg -q 'non-crash-resumable' "$source_skills_root/team/SKILL.md"
rg -q 'non-certification-gate' "$source_skills_root/team/SKILL.md"
rg -q 'cross-plan-perspective-missing' "$source_skills_root/team/SKILL.md"
rg -q 'same self-contained packet' "$source_skills_root/team/SKILL.md"
rg -Uq 'Each[[:space:]]+execute slice has exactly one implementer' "$source_skills_root/team/SKILL.md"
rg -q 'explicitly switch to ordinary Saving Team' "$source_skills_root/team/SKILL.md"
rg -q 'must not be reported as Cross' "$source_skills_root/team/SKILL.md"
rg -q 'per-lane decision, never a default fan-out' "$source_skills_root/team/SKILL.md"
rg -q 'cannot bypass the host/model allowlist' "$source_skills_root/team/SKILL.md"
rg -q 'Default Planning And Contract Review Mode' "$source_skills_root/team/SKILL.md"
rg -q 'Implementation-Stage Saving Mode' "$source_skills_root/team/SKILL.md"
rg -q 'Explicit Quality Mode' "$source_skills_root/team/SKILL.md"
rg -q 'expose_spawn_agent_model_overrides = true' "$ATLAS_FORGE_ROOT/plugins/atlas-workflow/README.md"
rg -q 'existing tasks do not hot-reload' "$ATLAS_FORGE_ROOT/plugins/atlas-workflow/README.md"
bash "$ATLAS_FORGE_ROOT/workflow/tests/contract_agent_model_policy.sh"
bash "$ATLAS_FORGE_ROOT/workflow/tests/contract_agent_model_catalog.sh"
bash "$ATLAS_FORGE_ROOT/workflow/tests/contract_zenmux_bearer_token.sh"
bash "$ATLAS_FORGE_ROOT/workflow/tests/contract_clarify_parallel_routing.sh"
bash "$ATLAS_FORGE_ROOT/workflow/tests/contract_team_cost_routing.sh"
bash "$ATLAS_FORGE_ROOT/workflow/tests/contract_team_review.sh"
rg -q "IMPLEMENTER_REPORT_JSON" "$ATLAS_FORGE_ROOT/.codex/agents/atlas-sdd-implementer.toml"
rg -q "IMPLEMENTER_REPORT_JSON" "$ATLAS_FORGE_ROOT/.codex/agents/atlas-sdd-implementer-deepseek.toml"
rg -q "REVIEW_VERDICT_JSON" "$ATLAS_FORGE_ROOT/.codex/agents/atlas-sdd-reviewer.toml"
rg -q "REVIEW_VERDICT_JSON" "$ATLAS_FORGE_ROOT/.codex/agents/atlas-sdd-phase-reviewer.toml"
...
rg -q "Decide whether intake is useful" "$source_skills_root/intake/SKILL.md"
...
rg -Uq "(?s)new or\s+evidence-challenged decision.*confirmation or\s+correction.*unchanged approved decisions without reopening" "$source_skills_root/intake/SKILL.md"
rg -Uq "per-decision confirmations, ask only about an unapproved decision or unresolved\s+gap that still remains" "$source_skills_root/intake/SKILL.md"
rg -Uq "(?s)every decision is explicitly approved and no gap\s+remains, do not ask for an extra shared-understanding confirmation.*grilling introduced the approved decisions" "$source_skills_root/intake/SKILL.md"
rg -q '\[Grilling Protocol\]\(#grilling-protocol\); do not add a separate final approval' "$source_skills_root/intake/SKILL.md"
! rg -Uq "grilling introduced a new decision or an unresolved gap" "$source_skills_root/intake/SKILL.md"
rg -Uq "(?s)grilling request and these\s+confirmations do not grant implementation authority" "$source_skills_root/intake/SKILL.md"
rg -Uq "Route by the actual gap:.*office-hours.*brainstorm.*clarify.*does not challenge the existing direction.*do not redo exploration" "$source_skills_root/analyze/SKILL.md"
rg -Uq "remaining gap explicitly requires bounded coordination.*Ordinary discussion or a staffing consideration does not.*formal machine review" "$source_skills_root/analyze/SKILL.md"
rg -Uq "(?s)analysis material available as input for further.*discussion or planning.*does not prove semantic sufficiency,\\s+implementation authority, or execution readiness.*current authoritative scope's required behavior.*permissions.*applicable acceptance.*concrete gap instead of claiming implementation-ready" "$source_skills_root/analyze/SKILL.md"
rg -Uq "(?s)real handoff is requested.*current Goal, confirmed\\s+decisions, and remaining gaps.*recommendations separately from\\s+approvals.*fixed checklist" "$source_skills_root/analyze/SKILL.md"
! rg -q "generic seven-field" "$source_skills_root/analyze/SKILL.md"
! rg -q "If the boundary is still unclear, switch to.*clarify" "$source_skills_root/analyze/SKILL.md"
! rg -q "only when a current Design Handoff exists" "$source_skills_root/brainstorm/SKILL.md"
rg -Uq "chosen direction remains valid.*do not wait for a Design Handoff for pure backend, CLI, or no-interaction work" "$source_skills_root/brainstorm/SKILL.md"
rg -Uq "(?s)behavior and\s+permission/authorization boundaries to preserve.*current confirmed\s+decisions" "$source_skills_root/brainstorm/SKILL.md" "$ATLAS_FORGE_ROOT/workflow/templates/brainstorm.md"
rg -Uq 'write `workflow/artifacts/<task-id>/spec.md` directly when the scope is simple, facts are stable, and no user-visible flow design is missing' "$source_skills_root/brainstorm/SKILL.md"
! rg -q 'When the discussion has become actionable enough.*also write a concise project doc' "$source_skills_root/brainstorm/SKILL.md"
! rg -q 'Write or update `workflow/artifacts/<task-id>/decision.md`:$' "$source_skills_root/brainstorm/SKILL.md"
rg -q "When a durable record is useful" "$source_skills_root/brainstorm/SKILL.md"
rg -q 'one current `workflow/artifacts/<task-id>/brainstorm.md`' "$source_skills_root/brainstorm/SKILL.md"
rg -q 'context.md.*factual base changes' "$source_skills_root/brainstorm/SKILL.md"
rg -Uq 'Write or update `workflow/artifacts/<task-id>/decision.md` only when it is' "$source_skills_root/brainstorm/SKILL.md"
rg -q 'distinct current decision body rather than a mirror' "$source_skills_root/brainstorm/SKILL.md"
rg -q 'When a durable handoff is genuinely useful' "$source_skills_root/brainstorm/SKILL.md"
rg -Uq 'Do not\s+create a repository bundle or copy the full workflow notes' "$source_skills_root/brainstorm/SKILL.md"
rg -q "Short Request Intake Gate" "$source_skills_root/worktree/SKILL.md"
rg -q "one-line request with complete context needs no fixed checklist" "$source_skills_root/clarify/SKILL.md"
rg -q "tiny escape hatch" "$source_skills_root/worktree/SKILL.md"
rg -q "Ask a question when its answer could change the recommendation" "$source_skills_root/clarify/SKILL.md"
rg -q "external issues, PRDs, or design docs" "$source_skills_root/worktree/SKILL.md"
rg -q "Decide whether Team is needed from the user's current request" "$source_skills_root/task/SKILL.md"
rg -q "Do not infer Team from task complexity, file count, or a default process" "$source_skills_root/task/SKILL.md"
rg -q "Follow .*atlas-workflow:task.*authoritative execution policy" "$source_skills_root/cw/SKILL.md"
rg -q "Match commit timing to the work phase" "$source_skills_root/task/SKILL.md" "$source_skills_root/team/SKILL.md"
...
rg -q "one current authoritative body" "$source_skills_root/brainstorm/SKILL.md"
rg -q "Keep one useful scope document" "$source_skills_root/clarify/SKILL.md"
rg -q "workflow docs bundle" "$source_skills_root/team-v1/SKILL.md"
rg -q "one current authoritative handoff body" "$source_skills_root/brainstorm/SKILL.md"
rg -Uq 'Create `contract-index.md` or a repo bundle only when handoff, audit, release or\s+existing project authority actually requires it' "$source_skills_root/clarify/references/contract-authoring.md"
...
rg -q "First-code guard" "$source_skills_root/clarify/references/contract-authoring.md"
rg -q "first_code_slice" "$ATLAS_FORGE_ROOT/workflow/templates/implementation-contract.md"
rg -q "first_code_owner" "$ATLAS_FORGE_ROOT/workflow/templates/implementation-contract.md"
...
rg -q "stop_if_no_code_by_phase" "$ATLAS_FORGE_ROOT/workflow/templates/implementation-contract.md" "$ATLAS_FORGE_ROOT/workflow/templates/implementation-contract.final.md"
...
rg -q "Product/UI gate" "$source_skills_root/clarify/references/contract-authoring.md"
...
rg -q "served_ui_validation_action" "$ATLAS_FORGE_ROOT/workflow/templates/implementation-contract.final.md"
...
rg -q "Risk-control rule" "$ATLAS_FORGE_ROOT/workflow/templates/implementation-contract.md"
rg -q "No separate safety-gate field is required" "$ATLAS_FORGE_ROOT/workflow/templates/implementation-contract.final.md"
! rg -q "required_safety_gates:" "$ATLAS_FORGE_ROOT/workflow/templates/implementation-contract.md" "$ATLAS_FORGE_ROOT/workflow/templates/implementation-contract.final.md"
rg -q "Concrete current risk controls, when applicable, are bound to named acceptance or edge-case evidence" "$ATLAS_FORGE_ROOT/workflow/templates/gate-checklist.md"
! rg -qi "hard safety gate|safety-gate evidence" "$ATLAS_FORGE_ROOT/workflow/templates/gate-checklist.md"
rg -q "Goal A" "$source_skills_root/team/references/business-acceptance.md"
...
rg -Uq 'Do not mirror scope into\s+`context.md`, `spec.md`, `decision.md` or a repo bundle' "$source_skills_root/clarify/SKILL.md"
...
rg -q "Product-Manager Progress Reports" "$source_skills_root/task/SKILL.md"
rg -q "Product-Manager Progress Reports" "$source_skills_root/team/SKILL.md"
rg -q 'a command name or green gate alone is not a capability explanation' "$source_skills_root/task/SKILL.md"
rg -q 'Structured agent output, ledgers, receipts, and raw logs remain internal evidence inputs' "$source_skills_root/team/SKILL.md"
rg -q "clean rewrite" "$source_skills_root/clarify/references/contract-authoring.md"
