...
assert_has "$TEAM" 'fresh child metadata proves' 'DeepSeek admission requires effective child metadata'
assert_has "$TEAM" 'provider-routing layer' 'provider routing failure is disclosed at the correct layer'
assert_has "$TEAM" 'native DeepSeek profiles use the exact host-admitted `high` effort' 'native DeepSeek effort follows the callable host contract'
assert_has "$TEAM" 'never sends `max` through native' 'standalone max is not mislabeled as a native max route'
...
assert_has "$TEAM" 'atlas-sdd-implementer-deepseek.*deepseek-v4-flash:deepseek.*high.*none' 'DeepSeek Flash is the exact host-admitted implementation alternative'
...
assert_has "$TEAM" 'atlas-sdd-explorer-deepseek.*deepseek-v4-flash:deepseek.*high.*none' 'DeepSeek Flash is the exact host-admitted exploration alternative'
assert_has "$TEAM" 'Codex-native child profiles deliberately use `high`' 'DeepSeek Flash native profiles use high explicitly'
