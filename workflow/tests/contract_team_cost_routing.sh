...
assert_has "$TEAM" 'atlas-sdd-implementer-deepseek.*deepseek-v4-flash:deepseek.*max.*none' 'DeepSeek Flash implementation always uses max'
...
assert_has "$TEAM" 'atlas-sdd-explorer-deepseek.*deepseek-v4-flash:deepseek.*max.*none' 'DeepSeek Flash exploration always uses max'
assert_has "$TEAM" 'Atlas always selects `max`' 'DeepSeek Flash native profiles use max explicitly'
