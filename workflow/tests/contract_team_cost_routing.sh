...
assert_has "$TEAM" 'rejects the selected DeepSeek profile.*`max` effort' 'host effort mismatch fails closed without downgrading max'
...
assert_has "$TEAM" 'atlas-sdd-implementer-deepseek.*deepseek-v4-flash:deepseek.*max.*none' 'DeepSeek Flash is the exact max-effort implementation alternative'
...
assert_has "$TEAM" 'atlas-sdd-explorer-deepseek.*deepseek-v4-flash:deepseek.*max.*none' 'DeepSeek Flash is the exact max-effort exploration alternative'
assert_has "$TEAM" 'native `max` effort' 'DeepSeek Flash profiles use the native max effort'
