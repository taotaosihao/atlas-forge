# Backend label cannot hide a fulfilled HTML MIME response
...
- served_ui_validation_action: For the backend API, page.route('/render', route => route.fulfill({headers: {'content-type': 'text/html'}, body: rawDocument})); page.goto(browser_entrypoint).
